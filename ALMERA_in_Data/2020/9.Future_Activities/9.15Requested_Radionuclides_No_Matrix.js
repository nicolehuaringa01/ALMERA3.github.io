// ALMERA_in_Data/2020/9.Future_Activities/9.15Requested_Radionuclides_No_Matrix.js

const csvDataPath15 = "/ALMERA3.github.io/data/2020_ALMERA_Capabilities_Survey.csv"; // Consistent CSV path
...
let selectedRequested_Radionuclides_No_Matrix = null; // Emulates Observable's mutable state
...
/**
 * Calculates the count of each Requested_Radionuclides_No_Matrix from the raw data.
...
/**
 * Gets the top N most frequently measured Requested_Radionuclides_No_Matrixs.
 * @param {number} n - The number of top Requested_Radionuclides_No_Matrixs to retrieve.
 * @returns {Array<Object>} Sorted array of top Requested_Radionuclides_No_Matrixs.
 */
const getTopRequested_Radionuclides_No_Matrixs = (n = 20) => {
    return Requested_Radionuclides_No_MatrixCountsData
...
/**
 * Creates a map from Requested_Radionuclides_No_Matrix to a nested map of MemberState to Set of LabNames.
 * @param {string} radionuclideColumn - The exact column name for radionuclides.
 * @returns {Map<string, Map<string, Set<string>>>} The mapping.
 */
const createRequested_Radionuclides_No_MatrixToLabsMap = (radionuclideColumn) => {
...
/**
 * Creates the Requested_Radionuclides_No_Matrixs Bar Chart.
 * @param {string} currentSelectedRequested_Radionuclides_No_Matrix - The currently selected Requested_Radionuclides_No_Matrix for highlighting.
 * @param {Function} onClickHandler - Callback function for bar clicks.
 * @returns {SVGElement} The D3 SVG node for the bar chart.
 */
const Requested_Radionuclides_No_MatrixsBarChart = (currentSelectedRequested_Radionuclides_No_Matrix, onClickHandler) => {
    const width = 928;
    const height = 500;
    const margin = { top: 40, right: 20, bottom: 120, left: 60 }; // Increased bottom margin for labels

    const x = d3.scaleBand()
        .domain(topRequested_Radionuclides_No_MatrixsData.map(d => d.name))
        .range([margin.left, width - margin.right])
        .padding(0.1);

    const y = d3.scaleLinear()
        .domain([0, d3.max(topRequested_Radionuclides_No_MatrixsData, d => d.value)]).nice()
        .range([height - margin.bottom, margin.top]);

    const color = d3.scaleOrdinal()
        .domain(topRequested_Radionuclides_No_MatrixsData.map(d => d.name))
        .range(d3.quantize(t => d3.interpolateSpectral(t * 0.8 + 0.1), topRequested_Radionuclides_No_MatrixsData.length).reverse());
    ...
    svg.append("g")
        .selectAll("rect")
        .data(topRequested_Radionuclides_No_MatrixsData)
        .join("rect")
        .attr("x", d => x(d.name))
        .attr("y", d => y(d.value))
        .attr("height", d => y(0) - y(d.value))
        .attr("width", x.bandwidth())
        .attr("fill", d => d.name === currentSelectedRequested_Radionuclides_No_Matrix ? "#FFD700" : color(d.name)) // Highlight selected
        .attr("stroke", d => d.name === currentSelectedRequested_Radionuclides_No_Matrix ? "black" : "none") // Add border to selected
        .attr("stroke-width", d => d.name === currentSelectedRequested_Radionuclides_No_Matrix ? 2 : 0)
        .on("click", (event, d) => {
            onClickHandler(d.name); // Call the handler to update selectedRequested_Radionuclides_No_Matrix
        })
        .append("title")
...
/**
 * Creates the Requested_Radionuclides_No_Matrixs Treemap.
 * @param {string} currentSelectedRequested_Radionuclides_No_Matrix - The currently selected Requested_Radionuclides_No_Matrix for highlighting.
 * @param {Function} onClickHandler - Callback function for tile clicks.
 * @returns {SVGElement} The D3 SVG node for the treemap.
 */
const Requested_Radionuclides_No_MatrixsTreemap = (currentSelectedRequested_Radionuclides_No_Matrix, onClickHandler) => {
    const width = 928;
    const height = 500;

    const data = {
        name: "Requested_Radionuclides_No_Matrixs",
        children: topRequested_Radionuclides_No_MatrixsData.map(d => ({ name: d.name, value: d.value }))
    };

    const color = d3.scaleOrdinal()
        .domain(topRequested_Radionuclides_No_MatrixsData.map(d => d.name))
        .range(d3.quantize(t => d3.interpolateBlues(t * 0.7 + 0.3), topRequested_Radionuclides_No_MatrixsData.length).reverse());
    ...
    node.append("rect")
        ...
        .attr("fill", d => d.data.name === currentSelectedRequested_Radionuclides_No_Matrix ? "#FFD700" : color(d.data.name)) // Highlight selected
        .attr("stroke", d => d.data.name === currentSelectedRequested_Radionuclides_No_Matrix ? "black" : "none") // Add border to selected
        .attr("stroke-width", d => d.data.name === currentSelectedRequested_Radionuclides_No_Matrix ? 2 : 0)
        .attr("width", d => d.x1 - d.x0)
        .attr("height", d => d.y1 - d.y0)
        .on("click", (event, d) => {
            onClickHandler(d.data.name); // Call the handler to update selectedRequested_Radionuclides_No_Matrix
        })
        .append("title")
...
const renderCharts_Requested_Radionuclides_No_Matrix = () => {
    ...
    if (selectedCharts.includes("Bar chart")) {
        const barChartSvg = Requested_Radionuclides_No_MatrixsBarChart(selectedRequested_Radionuclides_No_Matrix, chartClickHandler);
        chartDisplayContainer.node().appendChild(barChartSvg);
    }

    if (selectedCharts.includes("Tree Map")) {
        const treemapSvg = Requested_Radionuclides_No_MatrixsTreemap(selectedRequested_Radionuclides_No_Matrix, chartClickHandler);
