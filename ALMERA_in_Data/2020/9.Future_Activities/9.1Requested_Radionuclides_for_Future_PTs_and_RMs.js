// ALMERA_in_Data/2020/9.Future_Activities/9.1Requested_Radionuclides_for_Future_PTs_and_RMs.js

const csvDataPath1 = "/ALMERA3.github.io/data/2020_ALMERA_Capabilities_Survey.csv"; // Consistent CSV path
 ...
 */
const updateSelectedRequested_Radionuclides_for_Future_PTs_and_RMsLabs = async () => {
    ...
    if (!stateMap || stateMap.size === 0) {
        mapInfoDiv.append("p").html(`No labs found for <strong>${selectedRequested_Radionuclides_for_Future_PTs_and_RMs}</strong>.`);
        return;
    }
    ...
    if (allSurveyData_Requested_Radionuclides_for_Future_PTs_and_RMs.length > 0) {
        const headers = Object.keys(allSurveyData_Requested_Radionuclides_for_Future_PTs_and_RMs[0]);
    ...
    for (const [memberState, labsSet] of stateMap.entries()) {
        for (const labName of labsSet) {
            ...
            const row = allSurveyData_Requested_Radionuclides_for_Future_PTs_and_RMs.find(r =>
    ...
    if (labsForMap.length === 0) {
        mapInfoDiv.append("p").html(`No labs with valid geographic data found for <strong>${selectedRequested_Radionuclides_for_Future_PTs_and_RMs}</strong>.`);
        return;
    }
...
d3.csv(csvDataPath1).then(data => {
    allSurveyData_Requested_Radionuclides_for_Future_PTs_and_RMs = data;

    // Define the target column name for radionuclides for 9.2
    const targetRadionuclideColumnName = "9.1 Requested radionuclides and matrices for future PTs and RMs "; // **VERIFY THIS COLUMN NAME**
    ...
    document.querySelectorAll('#Requested_Radionuclides_for_Future_PTs_and_RMs-chart-selection-container .chart-selector-Requested_Radionuclides_for_Future_PTs_and_RMs').forEach(checkbox => {
    ...
    d3.select("#Requested_Radionuclides_for_Future_PTs_and_RMs-chart-display-container").html("<p style='color: red;'>Failed to load data for 9.2. Please check the CSV file path and content.</p>");
});
