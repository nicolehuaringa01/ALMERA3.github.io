...
function getFieldSurveyCapabilitiesCounts(data, FieldSurveyCapabilitiesColumn) {
    ...
    for (const row of data) {
        if (row[FieldSurveyCapabilitiesColumn]) {
            const FieldSurveyCapabilitiess = row[FieldSurveyCapabilitiesColumn]
                .split(/;|\r?\n/)   // split on ";" OR newlines
                .map(d => d.trim())
                .filter(d => d.length > 0);
            for (const aff of FieldSurveyCapabilitiess) {
                if (aff) counts.set(aff, (counts.get(aff) || 0) + 1); // Ensure FieldSurveyCapabilities string is not empty after trimming
            }
        }
    }
    let result = [];
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) otherCount += 1; // FieldSurveyCapabilitiess with only one occurrence go into "Other"
...
function getTopFieldSurveyCapabilitiess(FieldSurveyCapabilitiesCounts, numTop = 9) {
...
function renderBarChart(container, topFieldSurveyCapabilities, labsThatAnswered, color) {
    ...
    const x = d3.scaleLinear()
        .domain([0, d3.max(topFieldSurveyCapabilities, d => d.value)])
        .nice()
    ...
    svg.append("g")
        .selectAll("rect")
        .data(topFieldSurveyCapabilities)
        .join("rect")
            .attr("x", leftMargin)
            .attr("y", d => y(d.name))
...
async function initializeFieldSurveyCapabilitiesChart() {
    ...
    renderBarChart(container, topFieldSurveyCapabilities, labsThatAnswered, color);
}
// Run
