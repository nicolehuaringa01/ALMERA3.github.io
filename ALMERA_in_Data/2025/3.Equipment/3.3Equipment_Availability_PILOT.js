...
function getEquipment_AvailabilityCounts(data, Equipment_AvailabilityColumn) {
    ...
    for (const row of data) {
        if (row[Equipment_AvailabilityColumn]) {
            const Equipment_Availabilitys = row[Equipment_AvailabilityColumn]
                .split(/;|\r?\n/)   // split on ";" OR newlines
                .map(d => d.trim())
                .filter(d => d.length > 0);
            for (const aff of Equipment_Availabilitys) {
                if (aff) counts.set(aff, (counts.get(aff) || 0) + 1); // Ensure Equipment_Availability string is not empty after trimming
            }
        }
    }
    let result = [];
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) otherCount += 1; // Equipment_Availabilitys with only one occurrence go into "Other"
...
function getTopEquipment_Availabilitys(Equipment_AvailabilityCounts, numTop = 9) {
    let top = Equipment_AvailabilityCounts
        .slice()
        .sort((a, b) => d3.descending(a.value, b.value))
        .slice(0, numTop);
    const other = Equipment_AvailabilityCounts.find(d => d.name === "Other");
...
function renderBarChart(container, topEquipment_Availability, labsThatAnswered, color) {
    ...
    const x = d3.scaleLinear()
        .domain([0, d3.max(topEquipment_Availability, d => d.value)])
        .nice()
    ...
    svg.append("g")
        .selectAll("rect")
        .data(topEquipment_Availability)
        .join("rect")
            .attr("x", leftMargin)
            .attr("y", d => y(d.name))
...
async function initializeEquipment_AvailabilityChart() {
    ...
    const Equipment_AvailabilityColumn = "3.2 Select the equipment available in your laboratory.";
    if (!rawData[0] || !rawData[0][Equipment_AvailabilityColumn]) {
        return container.innerHTML = `<p style='color:red'>Missing "${Equipment_AvailabilityColumn}" column.</p>`;
    }

    const Equipment_AvailabilityCounts = getEquipment_AvailabilityCounts(rawData, Equipment_AvailabilityColumn);
    let topEquipment_Availability = Equipment_AvailabilityCounts.sort((a, b) => d3.descending(a.value, b.value)); // Corrected: Added sorting here
    ...
    renderBarChart(container, topEquipment_Availability, labsThatAnswered, color);
}
// Run
