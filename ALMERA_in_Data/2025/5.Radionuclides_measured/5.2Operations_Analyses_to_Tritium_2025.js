// ALMERA_in_Data/2025/5.Radionuclides_measured/5.2Operations_Analyses_to_Tritium_2025
...
function getOperations_Analyses_to_TritiumCounts(data, Operations_Analyses_to_TritiumColumn) {
    ...
    for (const row of data) {
        if (row[Operations_Analyses_to_TritiumColumn]) {
            const Operations_Analyses_to_Tritiums = row[Operations_Analyses_to_TritiumColumn]
                .split(/;|\r?\n/)   // split on ";" OR newlines
                .map(d => d.trim())
                .filter(d => d.length > 0);
            for (const aff of Operations_Analyses_to_Tritiums) {
                if (aff) counts.set(aff, (counts.get(aff) || 0) + 1); // Ensure Operations_Analyses_to_Tritium string is not empty after trimming
            }
        }
    }
    let result = [];
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) otherCount += 1; // Operations_Analyses_to_Tritiums with only one occurrence go into "Other"
...
function getTopOperations_Analyses_to_Tritiums(Operations_Analyses_to_TritiumCounts, numTop = 9) {
...
function renderBarChart(container, topOperations_Analyses_to_Tritium, labsThatAnswered, color) {
    ...
    const x = d3.scaleLinear()
        .domain([0, d3.max(topOperations_Analyses_to_Tritium, d => d.value)])
        .nice()
    ...
    svg.append("g")
        .selectAll("rect")
        .data(topOperations_Analyses_to_Tritium)
        .join("rect")
            .attr("x", leftMargin)
            .attr("y", d => y(d.name))
...
if (!Operations_Analyses_to_TritiumColumn) {
    ...
    renderBarChart(container, topOperations_Analyses_to_Tritium, labsThatAnswered, color);
}
// Run
