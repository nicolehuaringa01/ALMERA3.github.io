// ALMERA_in_Data/2025/6.Quality_Management_and_Reporting/6.13Databases_Used_to_Report_Data_Pie.js

const csvDataPath135 = "/ALMERA3.github.io/data/2025_ALMERA_Capabilities_Survey.csv";
...
function getDatabases_Used_to_Report_Data_PieCounts(data, Databases_Used_to_Report_Data_PieColumn) {
    ...
    for (const row of data) {
        ...
        if (row[Databases_Used_to_Report_Data_PieColumn]) {
            const databases = row[Databases_Used_to_Report_Data_PieColumn].split(/;|\n|\r/).map(d => d.trim());
...
async function initializeDatabases_Used_to_Report_Data_PieChart() {
    ...
    const targetColumnNameString = "6.10 Which database(s) is the routine monitoring data reported to? (Select all that apply)";
