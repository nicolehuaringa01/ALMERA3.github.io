...
function getOther_Networks_that_ALMERA_Labs_Belong_ToCounts(data, Other_Networks_that_ALMERA_Labs_Belong_ToColumn) {
    ...
    for (const row of data) {
        if (row[Other_Networks_that_ALMERA_Labs_Belong_ToColumn]) {
            const Other_Networks_that_ALMERA_Labs_Belong_Tos = row[Other_Networks_that_ALMERA_Labs_Belong_ToColumn]
                .split(/;|\r?\n/)   // split on ";" OR newlines
                .map(d => d.trim())
                .filter(d => d.length > 0);
            for (const aff of Other_Networks_that_ALMERA_Labs_Belong_Tos) {
                if (aff) counts.set(aff, (counts.get(aff) || 0) + 1); // Ensure Other_Networks_that_ALMERA_Labs_Belong_To string is not empty after trimming
            }
        }
    }
    let result = [];
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) otherCount += 1; // Other_Networks_that_ALMERA_Labs_Belong_Tos with only one occurrence go into "Other"
...
function getTopOther_Networks_that_ALMERA_Labs_Belong_Tos(Other_Networks_that_ALMERA_Labs_Belong_ToCounts, numTop = 9) {
...
function renderBarChart(container, topOther_Networks_that_ALMERA_Labs_Belong_To, labsThatAnswered, color) {
    ...
    const x = d3.scaleLinear()
        .domain([0, d3.max(topOther_Networks_that_ALMERA_Labs_Belong_To, d => d.value)])
        .nice()
    ...
    svg.append("g")
        .selectAll("rect")
        .data(topOther_Networks_that_ALMERA_Labs_Belong_To)
        .join("rect")
            .attr("x", leftMargin)
            .attr("y", d => y(d.name))
...
if (!Other_Networks_that_ALMERA_Labs_Belong_ToColumn) {
    ...
    return container.innerHTML = `<p style='color:red'>Missing 6.13 Select other networks in addition to ALMERA that the laboratory is member of column.</p>`;
}
    ...
    let topOther_Networks_that_ALMERA_Labs_Belong_To = Other_Networks_that_ALMERA_Labs_Belong_ToCounts.sort((a, b) => d3.descending(a.value, b.value)); // Corrected: Added sorting here

    if (topOther_Networks_that_ALMERA_Labs_Belong_To.length === 0) {
        return container.innerHTML = "<p>No data to display.</p>";
    }

    const labsThatAnswered = rawData.filter(d => d[Other_Networks_that_ALMERA_Labs_Belong_ToColumn] && d[Other_Networks_that_ALMERA_Labs_Belong_ToColumn].trim() !== "").length;

    const color = d3.scaleOrdinal()
        .domain(topOther_Networks_that_ALMERA_Labs_Belong_To.map(d => d.name))
        .range(d3.schemeTableau10);

    renderBarChart(container, topOther_Networks_that_ALMERA_Labs_Belong_To, labsThatAnswered, color);
}
// Run
