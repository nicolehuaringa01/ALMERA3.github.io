// ALMERA_in_Data/2025/6.Quality_Management_and_Reporting/6.2QMS_Basis.js
...
function getQMS_BasisCounts(data, QMS_BasisColumn) {
    ...
    for (const row of data) {
        if (row[QMS_BasisColumn]) {
            const QMS_Basiss = row[QMS_BasisColumn]
                .split(/;|\r?\n/)   // split on ";" OR newlines
                .map(d => d.trim())
                .filter(d => d.length > 0);
            for (const aff of QMS_Basiss) {
                if (aff) counts.set(aff, (counts.get(aff) || 0) + 1); // Ensure QMS_Basis string is not empty after trimming
            }
        }
    }
    let result = [];
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) otherCount += 1; // QMS_Basiss with only one occurrence go into "Other"
...
function getTopQMS_Basiss(QMS_BasisCounts, numTop = 9) {
...
function renderBarChart(container, topQMS_Basis, labsThatAnswered, color) {
    ...
    const x = d3.scaleLinear()
        .domain([0, d3.max(topQMS_Basis, d => d.value)])
        .nice()
...
async function initializeQMS_BasisChart() {
    ...
    renderBarChart(container, topQMS_Basis, labsThatAnswered, color);
}
// Run
