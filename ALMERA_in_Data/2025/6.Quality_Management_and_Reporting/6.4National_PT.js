// ALMERA_in_Data/2025/6.Quality_Management_and_Reporting/6.4National_PT.js
...
async function initializeNational_PTChart() {
    ...
    if (!container) {
      console.error("National_PT chart container element #National_PTs-chart-container not found.");
        const errorDiv = document.createElement('div');
        errorDiv.style.color = 'red';
        errorDiv.style.textAlign = 'center';
        errorDiv.textContent = 'Error: Chart container not found in HTML for National_PT chart.';
    ...
    try {
        ...
        console.log("National_PT CSV data loaded successfully. Number of records:", data.length);
    } catch (error) {
        console.error("Error loading National_PT CSV data:", error);
        container.innerHTML = "<p style='color: red; text-align: center;'>Failed to load National_PT data. Please check the console for details and ensure the CSV path is correct.</p>";
        return;
    }

    // --- Data Processing ---
    const National_PTColumn = '6.4 Has the laboratory participated in national proficiency testing exercises?';
    ...
    if (data.length === 0 || !data[0][National_PTColumn]) {
        console.error(`Error: CSV data is empty or missing expected column ("${National_PTColumn}").`);
        container.innerHTML = `<p style='color: red; text-align: center;'>Error: CSV data incomplete for National_PT chart. Check column name.</p>`;
        return;
    }

    data.forEach(d => {
        let answer = d[National_PTColumn];
    ...
    if (total === 0) {
        console.warn("No 'Yes' or 'No' responses found for National_PT.");
        container.innerHTML = "<p style='text-align: center;'>No data to display for National_PT.</p>";
        return;
    }
    ...
    console.log("Processed National_PT chartData:", chartData);
    ...
    const renderPlot = (currentWidth) => {
        ...
        const National_PTPlot = Plot.plot({
            width: currentWidth,
            height: height,
            y: {
                label: null,
        ...
        container.appendChild(National_PTPlot);
        console.log("National_PT chart appended to DOM.");
    };
...
document.addEventListener("DOMContentLoaded", initializeNational_PTChart);
