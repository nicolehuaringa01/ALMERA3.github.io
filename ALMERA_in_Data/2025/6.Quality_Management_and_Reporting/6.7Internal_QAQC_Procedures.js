// ALMERA_in_Data/2025/6.Quality_Management_and_Reporting/6.7Internal_QAQC_Procedures.js
...
async function initializeInternal_QAQC_ProceduresChart() {
    ...
    if (!container) {
      console.error("Internal_QAQC_Procedures chart container element #Internal_QAQC_Proceduress-chart-container not found.");
        const errorDiv = document.createElement('div');
        errorDiv.style.color = 'red';
        errorDiv.style.textAlign = 'center';
        errorDiv.textContent = 'Error: Chart container not found in HTML for Internal_QAQC_Procedures chart.';
    ...
    try {
        ...
        console.log("Internal_QAQC_Procedures CSV data loaded successfully. Number of records:", data.length);
    } catch (error) {
        console.error("Error loading Internal_QAQC_Procedures CSV data:", error);
        container.innerHTML = "<p style='color: red; text-align: center;'>Failed to load Internal_QAQC_Procedures data. Please check the console for details and ensure the CSV path is correct.</p>";
        return;
    }

    // --- Data Processing ---
    const Internal_QAQC_ProceduresColumn = '6.7 Has the laboratory established internal QA/QC procedures for measurement validation and to check equipment performance?';
    ...
    if (data.length === 0 || !data[0][Internal_QAQC_ProceduresColumn]) {
        console.error(`Error: CSV data is empty or missing expected column ("${Internal_QAQC_ProceduresColumn}").`);
        container.innerHTML = `<p style='color: red; text-align: center;'>Error: CSV data incomplete for Internal_QAQC_Procedures chart. Check column name.</p>`;
        return;
    }

    data.forEach(d => {
        let answer = d[Internal_QAQC_ProceduresColumn];
    ...
    if (total === 0) {
        console.warn("No 'Yes' or 'No' responses found for Internal_QAQC_Procedures.");
        container.innerHTML = "<p style='text-align: center;'>No data to display for Internal_QAQC_Procedures.</p>";
        return;
    }
    ...
    console.log("Processed Internal_QAQC_Procedures chartData:", chartData);
   ...
   const renderPlot = (currentWidth) => {
        ...
        container.appendChild(Internal_QAQC_ProceduresPlot);
        console.log("Internal_QAQC_Procedures chart appended to DOM.");
    };
