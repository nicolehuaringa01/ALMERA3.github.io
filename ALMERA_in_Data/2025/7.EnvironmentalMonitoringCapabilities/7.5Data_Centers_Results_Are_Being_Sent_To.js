...
        }
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) { // Data_Centers_Results_Are_Being_Sent_To with only one occurrence go into "Other"
            otherCount += 1;
        } else {
...
async function initializeAffiliationChart() {
    ...
    svg.append("text")
        ...
        .text(`Total responses: ${totalData_Centers_Results_Are_Being_Sent_ToCount.toLocaleString("en-US")}`);
