...
function gettargetCounts(data, targetColumn) {
    ...
    for (const row of data) {
        if (row[targetColumn]) {
            ...
            const targets = row[targetColumn].split(";").map(d => d.trim());
            for (const aff of targets) {
                if (aff) { // Ensure target string is not empty after trimming
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) { // targets with only one occurrence go into "Other"
            otherCount += 1;
        } else {
...
// This function selects the top N targets, including "Other" if present
function getToptargets(targetCounts, numTop = 6) {
...
async function initializetargetChart() {
    ...
    if (!container) {
        console.error("target chart container element #target-chart-container not found.");
        return;
    }
    ...
    try {
        ...
        container.innerHTML = "<p style='color: red; text-align: center;'>Failed to load target data. Check console for details (e.g., CSV path).</p>";
        return;
    }

    // --- Data Processing using the new functions ---
    const targetColumn = "1.13 Target users of laboratory's analytical services"; // User-provided column name
    if (!rawData[0] || !rawData[0][targetColumn]) {
        console.error(`Error: CSV data missing required column "${targetColumn}". Available columns:`, rawData.length > 0 ? Object.keys(rawData[0]) : "No data rows.");
        container.innerHTML = `<p style='color: red;'>Error: Missing "${targetColumn}" column in CSV data.</p>`;
        return;
    }

    const targetCounts = gettargetCounts(rawData, targetColumn);
    const toptarget = getToptargets(targetCounts, 6); // Get top 6 targets

    if (toptarget.length === 0) {
        console.warn("No valid target data found after processing.");
        container.innerHTML = "<p style='text-align: center;'>No target data to display after filtering/processing.</p>";
        return;
    }
    ...
    const color = d3.scaleOrdinal()
        ...
        .range(d3.quantize(t => d3.interpolateSpectral(t * 0.8 + 0.1), toptarget.length).reverse());

    // Create the pie layout and arc generator.
    const pie = d3.pie()
        .sort(null) // Do not sort, use the pre-sorted toptarget
        .value(d => d.value);
    ...
    console.log("target chart appended to DOM.");
