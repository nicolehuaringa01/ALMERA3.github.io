// js/6.Quality_Management_and_Reporting/6.16Other_Networks_that_ALMERA_Labs_Belong_To.js
...
function getOther_Networks_that_ALMERA_Labs_Belong_ToCounts(data, Other_Networks_that_ALMERA_Labs_Belong_ToColumn) {
    ...
    for (const row of data) {
        if (row[Other_Networks_that_ALMERA_Labs_Belong_ToColumn]) {
            ...
            const Other_Networks_that_ALMERA_Labs_Belong_Tos = row[Other_Networks_that_ALMERA_Labs_Belong_ToColumn].split(";").map(d => d.trim());
    ...
    for (const [name, value] of counts.entries()) {
        if (value === 1) { // Other_Networks_that_ALMERA_Labs_Belong_Tos with only one occurrence go into "Other"
            otherCount += 1;
        } else {
...
async function initializeOther_Networks_that_ALMERA_Labs_Belong_ToChart() {
    ...
    try {
        ...
        container.innerHTML = "<p style='color: red; text-align: center;'>Failed to load Other_Networks_that_ALMERA_Labs_Belong_To data. Check console for details (e.g., CSV path).</p>";
        return;
    }

    // --- Data Processing using the new functions ---
    const Other_Networks_that_ALMERA_Labs_Belong_ToColumn = "6.16 Select other networks in addition to ALMERA that the laboratory is member of."; // User-provided column name
    if (!rawData[0] || !rawData[0][Other_Networks_that_ALMERA_Labs_Belong_ToColumn]) {
        console.error(`Error: CSV data missing required column "${Other_Networks_that_ALMERA_Labs_Belong_ToColumn}". Available columns:`, rawData.length > 0 ? Object.keys(rawData[0]) : "No data rows.");
        container.innerHTML = `<p style='color: red;'>Error: Missing "${Other_Networks_that_ALMERA_Labs_Belong_ToColumn}" column in CSV data.</p>`;
        return;
    }
    ...
    const topOther_Networks_that_ALMERA_Labs_Belong_To = getTopOther_Networks_that_ALMERA_Labs_Belong_Tos(Other_Networks_that_ALMERA_Labs_Belong_ToCounts, 6); // Get top 6 Other_Networks_that_ALMERA_Labs_Belong_Tos
