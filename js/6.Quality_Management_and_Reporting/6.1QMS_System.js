...
async function initializeQMS_SystemChart() {
    const container = document.getElementById("QMS_System-chart-container");
    if (!container) {
      console.error("QMS_System chart container element #QMS_Systems-chart-container not found.");
        const errorDiv = document.createElement('div');
        errorDiv.style.color = 'red';
        errorDiv.style.textAlign = 'center';
        errorDiv.textContent = 'Error: Chart container not found in HTML for QMS_System chart.';
    ...
    try {
        ...
        console.log("QMS_System CSV data loaded successfully. Number of records:", data.length);
    } catch (error) {
        console.error("Error loading QMS_System CSV data:", error);
        container.innerHTML = "<p style='color: red; text-align: center;'>Failed to load QMS_System data. Please check the console for details and ensure the CSV path is correct.</p>";
        return;
    }

    // --- Data Processing ---
    const QMS_SystemColumn = '4.1 Has the laboratory been involved in development of routine and/or rapid analytical methods in preparedness for radiological emergency?';
    ...
    if (data.length === 0 || !data[0][QMS_SystemColumn]) {
        console.error(`Error: CSV data is empty or missing expected column ("${QMS_SystemColumn}").`);
        container.innerHTML = `<p style='color: red; text-align: center;'>Error: CSV data incomplete for QMS_System chart. Check column name.</p>`;
        return;
    }

    data.forEach(d => {
        let answer = d[QMS_SystemColumn];
    ...
    if (total === 0) {
        console.warn("No 'Yes' or 'No' responses found for QMS_System.");
        container.innerHTML = "<p style='text-align: center;'>No data to display for QMS_System.</p>";
        return;
    }
    ...
    console.log("Processed QMS_System chartData:", chartData);
    ...
    const renderPlot = (currentWidth) => {
        ...
        const QMS_SystemPlot = Plot.plot({
            width: currentWidth,
            height: height,
            y: {
                label: null,
        ...
        container.appendChild(QMS_SystemPlot);
        console.log("QMS_System chart appended to DOM.");
    };
...
document.addEventListener("DOMContentLoaded", initializeQMS_SystemChart);
