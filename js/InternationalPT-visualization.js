...
d3.csv("observable2020SurveyUpdatedData2025.csv").then(data => {
    // 1. RadMethodCounts (adapted from your Observable code)
    const RadMethodCounts = () => {
    ...
    // 2. topRadMethod (adapted)
    const topRadMethod = RadMethodCounts()
        .sort((a, b) => d3.descending(a.value, b.value))
        .slice(0, 10);

    // 3. equipmentToLabsMap (adapted)
    const RadMethodToLabsMap = () => {
        const map = new Map();

        for (const row of data) {
            const equipmentRaw = row['If "yes" above, state the name of the PT scheme/s'];
            ...
            const counts = {};
            for (const RadMethod of RadMethods) {
                if (!counts[RadMethod]) counts[RadMethod] = 0;
                counts[RadMethod]++;
            }

            for (const [RadMethod, count] of Object.entries(counts)) {
                if (!map.has(equipment)) map.set(RadMethod, new Map()); // equipment → Map<MemberState, Map<LabName, count>>

                const stateMap = map.get(RadMethod);
    ...
    let selectedRadMethod = null; // Mutable variable in Observable, now a regular variable

    // 4. EquipmentsPieChart (adapted)
    const RadMethodPieChart = () => {
        const width = 928;
        const height = Math.min(width, 500);

        const color = d3.scaleOrdinal()
            .domain(topRadMethod.map(d => d.name))
            .range(d3.quantize(t => d3.interpolateSpectral(t * 0.8 + 0.1), topRadMethod.length).reverse());
        ...
        svg.append("g")
            ...
            .attr("d", arc)
            .on("click", (event, d) => {
                selectedRadMethod = d.data.name; // Update the selected equipment
    ...
    const updateLabInfo = () => { // Renamed to updateLabInfo, since it's a function
        ...
        const stateMap = RadMethodToLabsMap().get(selected);
